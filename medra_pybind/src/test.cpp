...
int main(int argc, char **argv)
{
  ...
  if(SUCCEEDED(hr)){
    ...
    if (FAILED(hr))
      throw std::runtime_error("Failed to clear errors");
    SysFreeString(bstr1);
    ...
    if(SUCCEEDED(hr)){
      ...
      if(SUCCEEDED(hr)){
        ...
        if (FAILED(hr))
          throw std::runtime_error("Failed to take arm");
        SysFreeString(bstr1);
        VariantClear(&vntParam);
        VariantClear(&vntRet);
        ...
        if(SUCCEEDED(hr)){
          ...
          hr = bCap_RobotExecute(fd, hRob, bstr1, vntParam, &vntRet);
          if (FAILED(hr))
            throw std::runtime_error("Failed to turn on motor");
          SysFreeString(bstr1);
          VariantClear(&vntParam);
          VariantClear(&vntRet);
          ...
          hr = bCap_RobotExecute(fd, hRob, bstr1, vntParam, &vntRet);
          if (FAILED(hr))
            throw std::runtime_error("Failed to get current angles");
